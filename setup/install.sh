...
# @original_author Adam Eivy
# @mods Seth messer
###########################

DEFAULT_EMAIL="seth.messer@gmail.com"
DEFAULT_GITHUBUSER="megalithic"
...
sed -i 's/Seth Messer/'$firstname' '$lastname'/' .gitconfig > /dev/null 2>&1 | true
if [[ ${PIPESTATUS[0]} != 0 ]]; then
  echo
  running "looks like you are using OSX sed rather than gnu-sed, accommodating"
  sed -i '' 's/Seth Messer/'$firstname' '$lastname'/' .gitconfig;
  sed -i '' 's/seth.messer@gmail.com/'$email'/' .gitconfig;
  sed -i '' 's/megalithic/'$githubuser'/' .gitconfig;
  sed -i '' 's/replicant/'$(whoami)'/g' .zshrc;ok
else
  echo
